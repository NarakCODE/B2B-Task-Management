...
export const createCommentService = async (
  ...
  // Trigger Notification to Assignee (if not the commenter)
  if (task.assignedTo && task.assignedTo.toString() !== userId.toString()) {
    const senderUser = await UserModel.findById(userId)
    const senderName = senderUser?.name || "Someone"

    // Strip HTML/Markdown tags from preview content if needed, but a slice is fine
    const plainText = content.replace(/<[^>]*>/g, "") // strip simple HTML
    ...
    await createNotificationService({
      ...
      sender: userId,
      workspace: workspaceId,
      project: task.project?.toString() || null,
      task: taskId,
      type: "COMMENT",
      title: "New Comment Added",
      message: `${senderName} commented on task "${task.title}": "${previewText}"`,
    })
  }
